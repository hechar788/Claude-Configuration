...
- Review the project's existing patterns in `src/components`, `src/lib`, `src/hooks`, and `src/data`
...
- Consider the framework stack: React, TanStack (Query, Form, Table, Store), ShadCN/UI, Tailwind, Lucide icons
...
- **Data fetching:** Recommend TanStack Query patterns with proper query key factories
- **Forms:** Design around TanStack Form with Zod schema validation
- **Client state:** Evaluate whether TanStack Store is appropriate vs. component-local state
- **UI components:** Identify ShadCN/UI primitives to leverage before building custom solutions
- **Tables:** Consider TanStack Table for sortable/filterable data displays
...
5. **Test Strategy:** What tests need updating or creating, following the project's "tests next to source" pattern
